...
""")
    ...
    ser.timeout  = 0.1     # required so that the reader thread can exit
    ...
    ser1.timeout  = 0.1     # required so that the reader thread can exit
    ...
    ser2.timeout  = 0.1     # required so that the reader thread can exit
    ...
    while True:
        try:
            ...
            if ':' in data1 :
                ...
                if '\r\n' in data1 :
                    #list_data1 = data1.splitlines(True)
                    #sys.stderr.write('\nlist->%s,%s' % (len(list_data1),list_data1[0]))
                    data1a = data1[data1.index(':'):data1.index('\n')+1]
                    #data1=list_data1[len(list_data1)-1]
                    #enviar dato
                    #data = ser.read(1024)
                    ...
                    time.sleep(1)
                    while True :
                        time.sleep(time_no_response / 20)
                        n = ser.inWaiting()
