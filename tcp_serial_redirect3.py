...
banderasocket2 = 0
class Redirector:
    def __init__(self, serial_instance, socket, socket2, ser_newline=None, net_newline=None, spy=False):
        ...
        while self.alive:
            try:
                ...
                if data:
                    ...
                    try:
                        sys.stderr.write('reader A banderasocket: %s\n' % banderasocket)
                        if banderasocket == 1:
    ...
    def write(self, data):
        ...
        self._write_lock.acquire()
        try:
            sys.stderr.write('write(self, data)1')
            self.socket.sendall(data)
            sys.stderr.write('write(self, data)2')
        ...
        while self.alive:
            try:
                ...
                if banderasocket2 == 1:                        
                    ...
                    time.sleep(0.02)
                    self.serial.setRTS(True) 
                    sys.stderr.write('serial enviado 2: %s\n' % banderasocket)
                    # the spy shows what's on the serial port, so log it after converting newlines
                    if self.spy:
                        sys.stdout.write(codecs.escape_encode(data)[0])
                        sys.stdout.flush()                        
